...
const render = {
    ...
    bloques: function () {
        ...
        for (let cont = 0; cont < 5; cont++) {
            ...
            const i = (result < 0) ? (blockList.length) + result : (result > blockList.length - 1) ? (result - blockList.length) : result
            //if (i > blockList.length - 1) i = i - blockList.length;
            //if (i < 0) i = blockList.length + i;

            if (players[0].bloque == 205 && blockList[0].isTouching(players[0].x, players[0].y)) {
                players[0].bloque = 0;
                players[0].vuelta = players[0].vuelta + 1;
            }

            if (blockList[i] && i >= players[0].bloque && blockList[i].isTouching(players[0].x, players[0].y)) {
                players[0].bloque = i;
                lento = false;
            }

            //if (i < 0) i = blockList.length - 1;
            ctx.juego.rect((blockList[i].x * 20) / div + pX, (blockList[i].y * 20) / div + pY, (blockList[i].w * 20) / div, (blockList[i].h * 20) / div);
            blockList[i].c.forEach(e => {
...
const ball = {
    ...
    main: function () {
        if (!players[0].preso) {
            ...
            // Los controles del personaje
            if (keydown[87] == true) vY += -veloz; // w
            if (keydown[65] == true) vX += -veloz; // A
            if (keydown[83] == true) vY += veloz; // S
            if (keydown[68] == true) vX += veloz; // D
