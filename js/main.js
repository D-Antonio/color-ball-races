(function () {
  ...
  var pantalla = document.getElementById("pantalla"),
    pctx = pantalla.getContext("2d");
  var mapa = document.getElementById("mapa"),
    mctx = mapa.getContext("2d");
  var fondo = document.getElementById("fondo"),
    fctx = fondo.getContext("2d");
  var index0;
  ...
  var frameCount = 0,
    currentFps = 0,
    lasFrame = new Date().getTime(); //glovales para calcular los FPS
  var mm = new Image();
  mm.src = "img/m.png";
  var up = new Image();
  up.src = "img/v.png";
  ...
  Btn.prototype = {
    ...
    tocar: function (x, y) {
      if (
        x < (this.x * 20) / div ||
        x > (this.x * 20) / div + (this.width * 20) / div ||
        y < (this.y * 20) / div ||
        y > (this.y * 20) / div + (this.height * 20) / div
      ) {
        return false;
      }
      return true;
  ...
  game = {
    main: function () {
      ...
      for (var i = 0; i < r.length; i++) {
        ...
        document.getElementById("r").innerHTML +=
          ' <i class="border border-white rounded-circle fas fa-circle fa-fw" style="color: ' +
          r[i].c +
          ' "></i> <b>' +
          r[i].n +
          ' </b> | <i class="fas fa-heartbeat fa-fw"></i>' +
          r[i].v +
      ...
      mapa.width = mapa.width; // Borrar todo
      ...
      for (let cont = 0; cont < 4; cont++) {
        ...
        if (
          player[0].b == 205 &&
          game.bloque[0].tocar(player[0].x, player[0].y)
        ) {
          player[0].b = 0;
        ...
        if (
          game.bloque[i] != undefined &&
          i >= player[0].b &&
          game.bloque[i].tocar(player[0].x, player[0].y)
        ) {
          player[0].b = i;
          lento = false;
        }
        ...
        ctx.rect(
          (game.bloque[i].x * 20) / div + pX,
          (game.bloque[i].y * 20) / div + pY,
          (game.bloque[i].w * 20) / div,
          (game.bloque[i].h * 20) / div
        );
      ...
      if (player[0].p == 0) {
        ...
        if (keydown[87] == true || arriba_) vY += -veloz; // w
        if (keydown[65] == true || izquierda_) vX += -veloz; // A
        if (keydown[83] == true || abajo_) vY += veloz; // S
        if (keydown[68] == true || derecha_) vX += veloz; // D
      ...
      for (let x = 0; x < player.length; x++) {
        ...
        mctx.arc(
          game.bloque[player[x].b].x + w / 20 / 2,
          game.bloque[player[x].b].y + h / 20 / 2,
          game.round(h / 20 / div - h / 20 / 4 / 2),
          0,
          Math.PI * 2,
          true
        );
    ...
    cheket: function (t, i) {
      switch (t) {
        ...
        case 4:
          ...
          document.getElementById("p").innerHTML =
            objeto[cont].nom + " | " + objeto[cont].des;
    ...
    msg: function (ico, l) {
      ...
      /*toastr.error(
        '<b style="color: #fff">' + ico + "</b>",
        '<i style="color: #ff4400">System</i>',
    ...
    soket: function () {
      ...
      ws.onmessage = function (e) {
        let datos = e.data;
        let objeto = game.parseJSON(datos);
        //console.log(objeto);
        if (id == null) {
          id = objeto.SID;
          player.push({
        ...
        for (index1 = target_touches.length - 1; index1 > -1; --index1) {
          ...
          if (
            button.containsPoint(
              (touch.clientX - pantalla.getBoundingClientRect().left) * 1,
              (touch.clientY - pantalla.getBoundingClientRect().top) * 1
            )
          ) {
            if (index0 == 0) {
              izquierda_ = true;
    ...
    pistaControles: function () {
      ...
      game.botones.push(
        new Btn(
          pantalla.width * 0.08 - pantalla.height / 9,
          pantalla.height / 2 + pantalla.height / 9 - pantalla.height / 9,
          pantalla.height / 9,
          pantalla.height / 9
        )
      );
      ...
      game.botones.push(
        new Btn(
          pantalla.width * 0.67,
          pantalla.height / 2 + pantalla.height / 9,
          pantalla.height / 9,
          pantalla.height / 9
        )
      );
      game.botones.push(
        new Btn(
          pantalla.width * 0.67,
          pantalla.height / 2 + pantalla.height / 9 - (pantalla.height / 9) * 2,
          pantalla.height / 9,
          pantalla.height / 9
        )
      );
      game.botones.push(
        new Btn(
          pantalla.width * 0.67 - (pantalla.height / 9) * 2,
          pantalla.height / 2 + pantalla.height / 9 - pantalla.height / 9,
          pantalla.height / 9,
          pantalla.height / 9
        )
